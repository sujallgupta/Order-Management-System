          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            {lowStockList.length === 0 ? (
              ...
              <div style={{ width: '100%', overflow: 'hidden' }}>
                ...
                <div style={{ position: 'relative', width: '100%', height: `${lowStockList.length * 48 + 20}px` }}>
                  <svg width="100%" height="100%" style={{ overflow: 'visible' }}>
                    {lowStockList.map((prod, index) => {
                      const percentage = Math.min((prod.quantity / 10) * 100, 100);
                      const yPos = index * 48 + 10;
                      // Functional color indicator based on stock level
                      const activeBarColor = prod.quantity <= 3 ? 'var(--color-danger)' : 'var(--color-warning)';
                      
                      return (
                        <g key={prod.id}>
                          {/* Label */}
                          <text x="0" y={yPos + 14} fill="var(--text-primary)" fontSize="13" fontWeight="500">
                            {prod.name.length > 22 ? `${prod.name.substring(0, 22)}...` : prod.name}
                          </text>
                          {/* SKU */}
                          <text x="0" y={yPos + 28} fill="var(--text-muted)" fontSize="10">
                            {prod.sku}
                          </text>
                          {/* Bar background */}
                          <rect x="180" y={yPos + 6} width="55%" height="10" rx="3" fill="var(--border-color)" />
                          {/* Active Bar */}
                          <rect 
                            x="180" 
                            y={yPos + 6} 
                          ...
                          <text x="75%" y={yPos + 16} fill={activeBarColor} fontSize="12" fontWeight="600" textAnchor="start">
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div style={{ marginTop: '1rem', borderTop: '1px solid var(--border-color)', paddingTop: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
              <span style={{ fontSize: '0.8rem', fontWeight: '600', color: 'var(--text-secondary)' }}>Low Stock Products</span>
              <span className="badge warning">{summary.low_stock_count} Items</span>
            </div>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: '180px', overflowY: 'auto' }}>
              {lowStockList.map(p => (
                <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem', borderRadius: '4px', backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-color)' }}>
                  <div>
